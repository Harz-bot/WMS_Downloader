...
def crt_grid(zet, ebounds_minx, ebounds_miny, ebounds_maxx, ebounds_maxy,epsg, in_geom, bndl_geom):
    ...
    polygons = []
    names = []
    for x in cols[MINX:MAXX - 1]:
        x_unity = int((x - xmin) / length)
        for y in rows[MINY:MAXY - 1]:
            y_unity = int((y - ymin) / length)
            k_unity = str(x_unity) + '_' + str(y_unity)
            Xmax, Ymax = x + wide, y + wide

            polya = Polygon([(x, y), (x + wide, y), (x + wide, y + length), (x, y + length)])
            polyb = in_geom
            overlap = polyb.overlaps(polya)
            contain = polyb.contains(polya)
            overlap_bndl = bndl_geom.overlaps(polya)
            if contain is True or overlap is True or overlap_bndl is True:
                print('CONTAINS')
